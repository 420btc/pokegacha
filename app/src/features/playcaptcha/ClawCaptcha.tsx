...
function scatterPile(target: ToyId, pool: Array<{ toy: ToyId; w: number }>): Slot[] {
  const order = shuffle(pool)
  const rest = order.filter((t) => t.toy !== target)
  const tgt = order.find((t) => t.toy === target)!

  // floor row size adapts to pool length — keeps ~60% in front, rest nestled behind
  const nB = Math.max(8, Math.min(10, Math.ceil(order.length * 0.65)))
  const nTop = order.length - nB
  const bottomIdx = 2 + Math.floor(Math.random() * Math.min(4, nB - 4))
  ...
  const useGaps = shuffle(gaps).slice(0, Math.min(nTop, gaps.length))
  let ti = 0
  ...
  if (backCount > 0) {
    const tip = slots[nB + Math.floor(Math.random() * backCount)]
...
export function ClawCaptcha({
  ...
  const pool = poolProp ?? TOY_SET

  // unpinned challenges ask for a different toy every mount (stable within one)
  const [autoTarget] = useState<ToyId>(() => pool[Math.floor(Math.random() * pool.length)].toy)
  ...
  const pile = useMemo(() => scatterPile(target, pool), [target, pool])
    ...
    >
            ...
            {verified ? (
              ...
              <>
                ...
                <em style={{ color: t.accent }}>
                  <img className="clawcap-sub-toy" src={TOY_META[target].src} alt="" draggable={false} />
                  {t.label}
                </em>
              </>
            )}
      ...
      <div ref={machineEl} className="clawcap-machine">
        ...
        <img
          ...
          src={carried >= 0 ? TOY_META[pile[carried].toy].src : TOY_META[target].src}
          alt=""
          draggable={false}
          style={{
            width: carriedW,
